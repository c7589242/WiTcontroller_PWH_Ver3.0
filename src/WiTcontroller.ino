...
#include <TFT_eSPI.h>             // Replaces #include <U8g2lib.h>

// ---------------------------------------------------------------------------
// global display object (was previously a local variable in setup)
TFT_eSPI tft = TFT_eSPI(); // global display object
...
int getMultiThrottleIndex(char multiThrottle) {
    return multiThrottleIndexFromChar(multiThrottle);
}

char getMultiThrottleChar(int multiThrottleIndex) {
  return multiThrottleCharFromIndex(multiThrottleIndex);
}
...
void connectWitServer() {
  ...
  if (!client.connect(selectedWitServerIP, selectedWitServerPort)) {
    ...
    writeOledArray(false, false, true, true);
    writeOledBattery();
    // TFT_eSPI updates immediately, no need to sendBuffer()
...
void setup() {
  ...
  debug_println("TFT_eSPI Start"); 
  //  debug_println("u8g2 Start"); 
  // u8g2.setI2CAddress(0x3C * 2);
  // u8g2.setBusClock(100000);
  // global `tft` object already constructed above
  // u8g2.begin();
  // u8g2.firstPage();
  delay(1000);

  tft.init();
  tft.setRotation(1);       // 160x128 landscape mode
  tft.fillScreen(TFT_BLACK);
  // tft.initDMA();            // Enable DMA for smoother updates
...
void writeOledSpeed() {
  ...
  if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0 ) {
    writeOledFunctions();
     ...
     // throttle number
    // clear the little box first
    tft.fillRect(0,0,12,16,TFT_BLACK);

    // draw the number in default font size 1
    tft.setTextFont(1);
  ...
  uint16_t trackPowerColor = (trackPower == PowerOn) ? TFT_GREEN : ((trackPower == PowerOff) ? TFT_RED : TFT_YELLOW);
  tft.setTextFont(1);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawChar(1, 48, glyph_track_power, trackPowerColor, TFT_BLACK, 1);

  if (!heartbeatCheckEnabled) {
    tft.setTextFont(1);
    tft.drawChar(13, 49, glyph_heartbeat_off, TFT_WHITE, TFT_BLACK, 1);
    tft.drawLine(13, 48, 20, 41, TFT_WHITE);
    // alternate line option commented
    // tft.drawLine(13, 48, 21, 40, TFT_WHITE);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
  }
  ...
  tft.setTextFont(2); // bigger font for speed
  int width = tft.textWidth(cSpeed);
  tft.setCursor(22 + (55 - width), 45);
  tft.print(cSpeed);

  // speed and direction of next throttle
  if ( (maxThrottles > 1) && (foundNextThrottle) ) {
    tft.setTextFont(1);
    tft.setCursor(85+34,38);
    tft.print(sNextThrottleNo);
    tft.setCursor(85+12,48);
    tft.print(sNextThrottleSpeedAndDirection);
  }

  // no buffer to send for TFT_eSPI
...
void writeOledSpeedStepMultiplier() {
  if (speedStep != currentSpeedStep[currentThrottleIndex]) {
    // draw the X glyph and multiplier
    tft.setTextFont(1);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.drawChar(1, 38, glyph_speed_step, TFT_WHITE, TFT_BLACK, 1);
    tft.setCursor(9, 37);
...
void writeOledBattery() {
  ...
  if ( (useBatteryTest) && (showBatteryTest!=NONE) && (lastBatteryCheckTime>0)) {
    ...
    tft.setCursor(x, y);
    tft.print("Z");
    if (lastBatteryTestValue>10) tft.drawLine(x+1, y-6, x+1, y-3, TFT_WHITE);
    if (lastBatteryTestValue>25) tft.drawLine(x+2, y-6, x+2, y-3, TFT_WHITE);
    if (lastBatteryTestValue>50) tft.drawLine(x+3, y-6, x+3, y-3, TFT_WHITE);
    if (lastBatteryTestValue>75) tft.drawLine(x+4, y-6, x+4, y-3, TFT_WHITE);
    if (lastBatteryTestValue>90) tft.drawLine(x+5, y-6, x+5, y-3, TFT_WHITE);
...
void writeOledFunctions() {
  ...
  //   //    debug_print("Fn Off "); debug_println(i);
      ...
      // draw rounded indicator background in white
      tft.fillRoundRect(i*4+12,13,5,7,2,TFT_WHITE);
      // draw number in black on white
      tft.setTextFont(1);
      tft.setTextColor(TFT_BLACK, TFT_WHITE);
      tft.setCursor(i*4+13, 19);
      tft.print( (i<10) ? i : ((i<20) ? i-10 : i-20));
...
void writeOledArray(bool isThreeColums, bool isPassword, bool sendBuffer, bool drawTopLine) {
  ...
  for (int i=0; i < max; i++) {
    ...
    if (oledTextInvert[i]) {
      tft.fillRect(x, y-8, 62, 10, TFT_WHITE);
      tft.setTextColor(TFT_BLACK, TFT_WHITE);
    }
    tft.setCursor(x, y);
    tft.print(cLine1);
  ...
  if (drawTopLine) {
    tft.drawFastHLine(0,11,128,TFT_WHITE);
    writeOledBattery();
  }
  tft.drawFastHLine(0,51,128,TFT_WHITE);

  // no buffer;					// transfer internal memory to the display
...
void deepSleepStart(int shutdownReason) {
  ...
  // display power-save not required for TFT_eSPI
